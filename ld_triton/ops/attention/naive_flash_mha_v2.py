...
class _naive_mha_flash_v2(torch.autograd.Function):
    ...
    def forward(ctx, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, causal, sm_scale, BLOCK_M=128, BLOCK_N=128):
        ...
        L = torch.zeros((Z, H, N_CTX, 1), **factory_kwargs)         # [Z, H, N_CTX, 1]
        
        for z in range(Z):
            for h in range(H):
                for i in range(0, N_CTX, BLOCK_M):
                    q = Q[z, h, i: i+BLOCK_M, :]                                  # [BLOCK_M, HEAD_DIM]
                    o = torch.zeros((BLOCK_M, HEAD_DIM), **factory_kwargs)        # [BLOCK_M, HEAD_DIM]
                    l = torch.zeros((BLOCK_M, 1), **factory_kwargs)               # [BLOCK_M, 1]
                    m = torch.full((BLOCK_M, 1), float('-inf'), **factory_kwargs) # [BLOCK_M, 1]
                    ...
                    for j in range(0, N_CTX, BLOCK_N):
                        ...
                        if causal:
                            offs_m = torch.arange(i, i + BLOCK_M, device=Q.device)[:, None]   # [BLOCK_M, 1]
                            offs_n = torch.arange(j, j + BLOCK_N, device=Q.device)[None, :]   # [1, BLOCK_N]
                            s[offs_m < offs_n] = torch.finfo(s.dtype).min                     # [BLOCK_M, BLOCK_N]

                        m_new = torch.max(m, torch.max(s, dim=-1, keepdim=True)[0])           # [BLOCK_M, 1]
                        p = torch.exp(s.float() - m_new).to(q.dtype)                          # [BLOCK_M, BLOCK_N]
                        l_new = torch.exp(m - m_new) * l + torch.sum(p, dim=-1, keepdim=True) # [BLOCK_M, 1]
                        o_new = torch.exp(m - m_new) * o + torch.matmul( p, v)                # [BLOCK_M, HEAD_DIM]
                        o = o_new # [BLOCK_M, BLOCK_N]
                        l = l_new # [BLOCK_M, 1]
                        m = m_new # [BLOCK_M, 1]

                    O[z, h, i: i+BLOCK_M] = (1.0 / l) * o        # [BLOCK_M, HEAD_DIM]
                    L[z, h, i: i+BLOCK_M, :] = m + torch.log(l)  # [BLOCK_M, 1]

        # for i in range(0, N_CTX, BLOCK_M):
        #     q = Q[:, :, i: i+BLOCK_M, :]                                        # [Z, H, BLOCK_M, HEAD_DIM]
        #     o = O[:, :, i: i+BLOCK_M, :]                                        # [Z, H, BLOCK_M, HEAD_DIM]
        #     l = L[:, :, i: i+BLOCK_M, :]                                        # [Z, H, BLOCK_M, 1]
        #     m = torch.full((Z, H, BLOCK_M, 1), float('-inf'), **factory_kwargs) # [Z, H, BLOCK_M, 1]
        #     for j in range(0, N_CTX, BLOCK_N):
        ...
        #             offs_m = torch.arange(i, i + BLOCK_M, device=Q.device)[:, None]   # [BLOCK_M, 1]
        #             offs_n = torch.arange(j, j + BLOCK_N, device=Q.device)[None, :]   # [1, BLOCK_N]
        #             s[:, :, offs_m < offs_n] = torch.finfo(s.dtype).min               # [Z, H, BLOCK_M, BLOCK_N]

        #         m_new = torch.max(m, torch.max(s, dim=-1, keepdim=True)[0])            # [Z, H, BLOCK_M, 1]
        #         p = torch.exp(s.float() - m_new).to(q.dtype)                           # [Z, H, BLOCK_M, BLOCK_N]
                
        #         l_new = torch.exp(m - m_new) * l + torch.sum(p, dim=-1, keepdim=True)  # [Z, H, BLOCK_M, 1]
        #         o_new = torch.exp(m - m_new) * o + torch.matmul( p, v)                 # [Z, H, BLOCK_M, 1]
        #         o = o_new # [Z, H, BLOCK_M, 1]
        #         l = l_new # [Z, H, BLOCK_M, 1]
        #         m = m_new # [Z, H, BLOCK_M, 1]

        #     O[:, :, i: i+BLOCK_M] = (1.0 / l) * o        # [Z, H, BLOCK_M, HEAD_DIM]
        #     L[:, :, i: i+BLOCK_M, :] = m + torch.log(l)  # [Z, H, BLOCK_M, 1]
    ...
    def backward(ctx, dO):
        ...
        BLOCK_M = ctx.BLOCK_M
        BLOCK_N = ctx.BLOCK_N
        dQ = torch.zeros_like(Q)                    # [Z, H, N_CTX, HEAD_DIM]
        dK = torch.zeros_like(K)                    # [Z, H, N_CTX, HEAD_DIM]
        dV = torch.zeros_like(V)                    # [Z, H, N_CTX, HEAD_DIM]
        D = torch.sum(dO * O, dim=-1, keepdim=True) # [Z, H, N_CTX, 1]
        for z in range(Z):
            for h in range(H):
                for j in range(0, N_CTX, BLOCK_N):
                    k = K[z, h, j: j+BLOCK_N, :]                            # [BLOCK_N, HEAD_DIM]
                    v = V[z, h, j: j+BLOCK_N, :]                            # [BLOCK_N, HEAD_DIM]
                    dk = torch.zeros((BLOCK_N, HEAD_DIM), **factory_kwargs) # [BLOCK_N, HEAD_DIM]
                    dv = torch.zeros((BLOCK_N, HEAD_DIM), **factory_kwargs) # [BLOCK_N, HEAD_DIM]
                    for i in range(0, N_CTX, BLOCK_M):
                        q  =  Q[z, h, i: i+BLOCK_M, :] # [BLOCK_M, HEAD_DIM]
                        ...
                        p = torch.exp(s.float() - l).to(q.dtype)    # [BLOCK_M, BLOCK_N]
        ...
        #     k = K[:, :, j: j+BLOCK_N, :]                              # [Z, H, BLOCK_N, HEAD_DIM]
        #     v = V[:, :, j: j+BLOCK_N, :]                              # [Z, H, BLOCK_N, HEAD_DIM]
        #     dk = torch.zeros((BLOCK_N, HEAD_DIM), **factory_kwargs)   # [Z, H, BLOCK_N, HEAD_DIM]
        #     dv = torch.zeros((BLOCK_N, HEAD_DIM), **factory_kwargs)   # [Z, H, BLOCK_N, HEAD_DIM]
        #     for i in range(0, N_CTX, BLOCK_M):
        #         q  =  Q[:, :, i: i+BLOCK_M, :]  # [Z, H, BLOCK_M, HEAD_DIM]
        #         o  =  O[:, :, i: i+BLOCK_M, :]  # [Z, H, BLOCK_M, HEAD_DIM]
        ...
        #         l  =  L[:, :, i: i+BLOCK_M, :]  # [Z, H, BLOCK_M, 1]
        #         d  =  D[:, :, i: i+BLOCK_M, :]  # [Z, H, BLOCK_M, 1]
        #         s = torch.matmul(q, k.permute(0, 1, 3, 2)) * sm_scale               # [Z, H, BLOCK_M, BLOCK_N]
        #         if causal:
        #             offs_m = torch.arange(i, i + BLOCK_M, device=Q.device)[:, None] # [BLOCK_M, 1]
        #             offs_n = torch.arange(j, j + BLOCK_N, device=Q.device)[None, :] # [1, BLOCK_N]
        #             s[:, :, offs_m < offs_n] = torch.finfo(s.dtype).min             # [Z, H, BLOCK_M, BLOCK_N]
        #         p = torch.exp(s.float() - l).to(q.dtype)           # [Z, H, BLOCK_M, BLOCK_N]
