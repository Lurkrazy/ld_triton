...
class NaiveRingAllReduce(torch.nn.Module):
    ...
    def _reduce_backward_hook(self, m: torch.nn.Module, grad_input, grad_output):
        ...
        for p in params:
            ...
            if p.grad is None:
                continue
            padding_size = int((p.grad.numel() + self._world_size - 1) // self._world_size) * self._world_size
            scatter_grad = torch.zeros(
                padding_size // self._world_size, 
            ...
            input_grad = torch.zeros(
                ...
                dtype=p.grad.dtype, 
                device=p.grad.device)
            input_grad.narrow(0, 0, p.grad.numel()).copy_(p.grad.view(-1))
            dist.reduce_scatter_tensor(scatter_grad, input_grad, group=self._group, op=dist.ReduceOp.SUM)
            dist.all_gather_into_tensor(input_grad, scatter_grad, group=self._group)
            shape = p.grad.shape
